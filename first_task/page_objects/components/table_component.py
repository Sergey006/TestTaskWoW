...
class TableComponent(BaseComponent):
    ...
    def get_row_cells(self, row_number):
    ...
    def get_row_values_mapping(self, row_number):
        ...
        row_elements = self.get_row_cells(row_number)
        for x in range(0, len(header_elements)):
            title_value_map[extract_text(header_elements[x])] = extract_text(row_elements[x])
    ...
    def get_rows_quantity(self):
        elements = self.browser.find_elements(self.all_body_rows_cells_locator)
        return len(elements)
